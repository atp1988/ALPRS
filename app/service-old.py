...
class Inference:
    ...
    @staticmethod
    def is_similar(image, path, threshold=SIMILARITY_THRESHOLD):
        sim_list = []
        files = os.listdir(path)
        gray1 = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        for file in files:
            image2 = cv2.imread(os.path.join(path, file))
            ...
            # محاسبه میزان شباهت با استفاده از همبستگی
        ...
        if len(boxes):
            ...
            for res, box in zip(results, boxes):
                if len(res) == 8:
                    ...
                    if res not in os.listdir('cropped-plate'):
